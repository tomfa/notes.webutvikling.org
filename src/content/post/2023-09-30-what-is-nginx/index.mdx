...
[A reverse proxy](https://www.nginx.com/blog/reverse-proxy-using-nginx-plus/) accepts requests from the internet and routes it to the server that should reply. Like a bastion host, it can then be a single point of security breaches, so it may be hardened, while other servers can "relax" internally.
