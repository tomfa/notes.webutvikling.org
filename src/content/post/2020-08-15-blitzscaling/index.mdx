...
tags: [scaling, startup, Reid Hoffman]
category: book
description: "Splurge money to grow insanely fast. Only do it when speed into the market is the whole game."
---
...
Efficiency in this book is about value per monetary input. One example he mentions would be to pay construction workers to work around the clock, to finish a work-site in 3 months instead of 6, implicitly paying them a lot _more money to do the same thing_. It's not about efficiency per employee hour, but efficiency per dollar.

I found the way the term a bit confusing, so I think of "sacrifice efficiency" as "splurging money".

### Classic start up growth

> _Classic start up growth_ prioritizes efficiency in the face of uncertainty.
...
> _Classic scale-up growth_ focuses on growing efficiently once certainty is achieved about the environment.
...
This is how classic scale-up focuses on efficiency to increase profitability.

### Fastscaling

> _Fastscaling_ means I'm willing to sacrifice efficiency for increased growth rate.
...
> _Blitzscaling_ means that you're willing to sacrifice efficiency for speed, without waiting for certainty.
...
To blitzscale, you should find a way to spread organically, preferably exponentially. Below are three examples on how that has been done previously.
...
- One of Paypals vehicle for distribution was settling purchases on eBay. This is an example of leveraging an existing network (eBay), to grow quickly. (On this: allowing users to send money without signing up, or sending money directly to a bank account would've been bad.)
...
- **Two-Sided network effects**: E.g. Uber. Each driver makes Uber more valuable for paying customers and vice versa.
...
> ... you need to have a big new opportunity - one where the market size and gross margins intersect to create enormous potential value, and there isn't an existing dominant market leader.
...
> Blitzscaling is by definition inefficient use of resources, and it only makes sense when speed and momentum are important.
...
> Ubers significant investments in autonomous vehicle technology could eliminate its biggest expense (driver payment) in one fell swoop.
...
> ...embrace counterintuitive rules like hiring "good enough" people, launching flawed and imperfect products, letting fires burn and ignore angry customers.
...
> Operational scalability is one of the primary growth limiters that scale-ups need to address. When a business can grow revenues faster than the number of employees without collapsing under its own growth, they can achieve greater profitability and keep growing with less constraints.
...
> AWS reflects one of the ways that Amazon has made operational scalability a competitive advantage: _the power of modularity_.
...
- **Writing code in independent modules/libraries**, so they can later be turned into an API, scaled or replaced independently.
...
> As Facebook grew, Mark changed the philosophy to "Move fast and break things _with stable infrastructure_"
