...
pubDate: 2021-04-23
heroImage: /images/lukas-blazek-mcSDtbWXUZU-unsplash.jpg
...
Similarly if you have a lot of metrics – it can become too much and we become blind to them.
...
If you don't use Heroku, logz.io has a ton of [log integrations](https://app-uk.logz.io/#/dashboard/send-your-data/log-sources) with good documentation. A popular option for my crowd would be the [NodeJS logger](https://app-uk.logz.io/#/dashboard/send-your-data/log-sources/nodejs).
...
![Service is mapped as text](/images/service-is-text.png)
...
![Set index patterns](/images/setting-index-patterns.png)

Then, search for status, edit it and change its type to integer

![Changing status type to integer](/images/change-status-type.png)
...
![Create new dashboard](/images/dashboard-add.png)

Repeat it again for the "HTTP traffic". We now got a simple performance dashboard! 😲

![Initial performance dashboard](/images/dashboard-start.png)
...
![Edit visualization](/images/dashboard-edit-vis.png)

On the left hand side, add a Y axis with median aggregation on the service field.

![Add Response time aggregation](/images/add-y-axis.png)
...
![Change y axis](/images/vis-change-y-axis-=metrics.png)

Done! The graph shows both traffic count (left axis) and median response time (right axis)
![Traffic graph](/images/vis-2-y-axis.png)
...
Again, select the Bucket X-axis to be a Date histogram based on timestamp

![Selecting date histogram for the X-axis](/images/buckets-date-histogram.png)

Now filter based on status being above 400 – we don't care about successful requests here.

![Filter HTTP status above 400](/images/vis-filter-above-400.png)

Under Buckets, add a "Split series" sub bucket.

![Split buckets for each status code](/images/vis-split-series.png)
...
![Splitting bucket by url](/images/table-split-bucket.png)

We now got a table showing us the most requested urls.

![Most popular urls](/images/table-url-count.png)

Now we can add a new metric, for the percentiles of response times.
Add a metric on the left side for Percentiles aggregation of "service" field, on 50, 90 and 98 percents.

![Splitting response time percentiles](/images/table-response-time-percentiles.png)

Voila, we got ourselves a table of slow urls. A great place to start if our performance is bad 🕵️‍♀️

![Table visualization](/images/table-vis.png)
...
![Splitting bucket by url ordered by count](/images/vis-split-bucket-count.png)
...
![Filter away any OK status](/images/vis-filter-above-400.png)
...
![Sum filter aggregation](/images/metrics-bucket-sum-aggregatgion.png)

_Repeat the process for the different statuses you care about to add more columns._

Tada! 🎉 You now got a list of URLs that responds with bad status codes, and their amount. You can click on each header column to sort by e.g. most 500 errors, most 404 errors etc.

![View of debugging list](/images/debugging-list.png)
