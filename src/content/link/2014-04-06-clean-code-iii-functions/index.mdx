...
url: http://vimeo.com/12643301
tags: [clean code, Robert C. Martin]
description: "Functions should do 1 thing. And be short. With few args. Without comments."
---
...
**EITHER **your function returns a value **OR **it does something. This might not always be possible to make it so, but at least **TRY. **The example Roberts mentions a method that is called checkPassword or something, and also happens to initialize a session (which would be applicable the first time, but break stuff later). Return a value OR do something.
